...
def Almond_Qual(frame):
    try:
        ...
        blu_lower = np.array([0, 0, 0], np.uint8)
        blu_upper = np.array([255, 129, 255], np.uint8)

        # the range of soaked almond
        dry_lower = np.array([16, 0, 229], np.uint8)
        ...
        for contour in contours1:
            ...
            if c >= 9000:  # soaked almond
                x, y, w, h = cv2.boundingRect(contour)
                soked_bounding.append([x, y, w, h])
                print('socked_lenght:', len(soked_bounding))
        for contour in contours:
            c = cv2.contourArea(contour)
            if 11500 <= c <= 18000:  # dry class
                # if c >= 11500: #dry almond
                x, y, w, h = cv2.boundingRect(contour)
                dry_bounding.append([x, y, w, h])
                # print('dry_lenght:', len(dry_bounding))
            elif 2000 <= c <= 10500:  # broken class
                x, y, w, h = cv2.boundingRect(contour)
                broken_bounding.append([x, y, w, h])
                # print('broken_lenght:', len(broken_bounding))
        ...
        for soaked in soked_bounding:
            for broken in broken_bounding:
                broken_x, broken_y = (broken[0] + (broken[0] + broken[2])) // 2, (
                            broken[1] + (broken[1] + broken[3])) // 2
                soaked_x, soaked_y = (soaked[0] + (soaked[0] + soaked[2])) // 2, (
                            soaked[1] + (soaked[1] + soaked[3])) // 2
        ...
        print('final soaked:', len((soked_bounding)))

        for broken in broken_bounding:
            cv2.rectangle(frame, (broken[0], broken[1]), (broken[0] + broken[2], broken[1] + broken[3]), (0, 0, 255), 2)
            cv2.putText(frame, "defected", (broken[0] - 5, broken[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        print('final defected:', len((broken_bounding)))
        ...
        print('final dry:', len((dry_bounding)))

        total_count = len((soked_bounding)) + len((broken_bounding)) + len((dry_bounding))
        print(
            "Total Almond={} , Soaked Almond={}, Dry Almond={},Broken Almond={}".format(total_count,
                                                                                        len((soked_bounding)),
                                                                                        len((dry_bounding)),
                                                                                        len((broken_bounding))))
        ...
        Dsoaked = len((soked_bounding)) / total_count
        persoaked = Dsoaked * 100
        format_persoaked = '{:.2f}'.format(persoaked)
        print("soaked percent", format_persoaked)
        cv2.putText(frame, "Soaked Almond ={} %".format(format_persoaked), (20, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255), 2)

        Ddry = len((dry_bounding)) / total_count
        perdry = Ddry * 100
        format_perdry = '{:.2f}'.format(perdry)
        print("dry percent", format_perdry)
        cv2.putText(frame, "Dry Almond ={} %".format(format_perdry), (20, 55), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255),
                    2)

        Dbad = len((broken_bounding)) / total_count
        perbad = Dbad * 100
        format_perbad = '{:.2f}'.format(perbad)
        print("defected percent", format_perbad)
        cv2.putText(frame, "Defected Almond ={} %".format(format_perbad), (20, 85), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255), 2)

        ## calculation of batch lot: good and bad
        #print("percentage", perdry)
        #print("percentage of length", len((perdry)))
        if (perdry) >= 80.00:
            cv2.putText(frame, "Passed Batch", (20, 385), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (0, 255, 0), 2)

        else:
            cv2.putText(frame, "Failed Batch", (20, 385), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        (0, 0, 255), 2)
